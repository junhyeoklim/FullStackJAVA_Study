	...
	public void inputPhoneInfo(PhoneInfo pInfo) {
		...
		for (i = 0; i < size; i++) {			
			if (pInfoArr.get(i).getName().compareTo(pInfo.getName()) > 0) {
				...
				//					pinfo = (PhoneInfo)pInfoArr.get(j);
				//					pInfoArr.add(j+1, pinfo);
				//					 }
				break;
			}
		}
		pInfoArr.add(i, pInfo);
		/* cntOfPhone++; */
	}
...
class PhoneUI {
	...
	private static PhoneBook pb = PhoneBook.getPhoneBookInst(10);
