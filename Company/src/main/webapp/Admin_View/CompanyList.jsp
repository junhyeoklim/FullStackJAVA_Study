					...
					<c:forEach var="dto" items="${requestScope.list}">
						<div class="col-md-4 mb-4">
							<div class="card h-100">
								<div class="card-body">
									<h5 class="card-title">${dto.s_name}</h5>
									<p class="card-text">
										<strong>부서:</strong> ${dto.s_department}<br>
										<strong>직급:</strong> ${dto.s_rank}<br>
										<strong>이메일:</strong> ${dto.s_mail}<br>
										<strong>전화번호:</strong> ${dto.s_phoneNumber}
									</p>
								</div>
								<div class="card-footer text-center">
									<a href="${contextPath}/modify.do?s_id=${dto.s_id}" class="btn btn-primary">수정</a>
								</div>
							</div>
						</div>
					</c:forEach>
